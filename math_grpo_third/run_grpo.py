...
def make_conversation(example):
    raw_content = example["messages"][0]["content"]
    # few-shot 예시 버리고 마지막 Question만 추출
    problem_text = raw_content.split("Question:")[-1].strip()
    return {
        "prompt": [
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user", "content": USER_PROMPT_TEMPLATE.format(problem=problem_text)},
        ],
    }
...
if __name__ == "__main__":
    ...
    beta = args.beta
    # --- 데이터셋 로드 ---
    ds1_id = "allenai/RLVR-GSM"
    ds2_id = "allenai/RLVR-MATH"
